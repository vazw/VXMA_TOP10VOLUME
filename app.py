...
logging.basicConfig(
    filename="log.log", format="%(asctime)s - %(message)s", level=logging.DEBUG
)
print("VXMA bot (Form Tradingview)By Vaz.")
print(
    "Donate XMR : 87tT3DZqi4mhGuJjEp3Yebi1Wa13Ne6J7RGi9QxU21FkcGGNtFHkfdyLjaPLRv8T2CMrz264iPYQ2dCsJs2MGJ27GnoJFbm"  # noqa:
)
# call config
...
wellcome = (
    ...
    + "\nBasic Setting\n----------\nRisk : "
    + str(RISK)
    + "\nBot Will Stop Entry when balance < "
    + str(min_balance)
    + "\nGOODLUCK"
)
...
def candle(df, symbol, tf):
    ...
    color = mplf.make_marketcolors(
        up="green", down="red", volume="inherit", wick="white", edge="white"
    )
...
def RRTP(df, symbol, direction, step):
    ...
    if direction:
        ...
        if step == 1:
            target = bid * (
                1 - ((float(swinghigh) - bid) / bid) * float(TPRR1)
            )
        if step == 2:
            target = bid * (
                1 - ((float(swinghigh) - bid) / bid) * float(TPRR2)
            )
    return target
...
def OpenLong(df, balance, symbol, lev):
    ...
    if free > min_balance:
        try:
            order = exchange.createMarketOrder(
                symbol, "buy", amount, params={"positionSide": Lside}
            )
            logging.info(order)
        ...
        if USESL:
            if currentMODE["dualSidePosition"]:
                orderSL = exchange.create_order(
                    ...
                    float(swinglow),
                    params={
                        "stopPrice": float(swinglow),
                        "triggerPrice": float(swinglow),
                        "positionSide": Lside,
                    },
                )
                if Tailing_SL:
                    ordertailingSL = exchange.create_order(
                        symbol,
                        "TRAILING_STOP_MARKET",
                        "sell",
                        amount,
                        params={
                            "activationPrice": float(RR1(df, symbol, True)),
        ...
        if USETP:
            orderTP = exchange.create_order(
                symbol,
                "TAKE_PROFIT_MARKET",
                "sell",
                amttp1,
                float(RRTP(df, symbol, True, 1)),
                params={
                    "stopPrice": float(RRTP(df, symbol, True, 1)),
                    "triggerPrice": float(RRTP(df, symbol, True, 1)),
                    "positionSide": Lside,
                },
            )
            orderTP2 = exchange.create_order(
                symbol,
                "TAKE_PROFIT_MARKET",
                "sell",
                amttp2,
                float(RRTP(df, symbol, True, 2)),
                params={
                    "stopPrice": float(RRTP(df, symbol, True, 2)),
                    "triggerPrice": float(RRTP(df, symbol, True, 2)),
                    "positionSide": Lside,
                },
            )
...
def OpenShort(df, balance, symbol, lev):
    ...
    if free > min_balance:
        try:
            order = exchange.createMarketOrder(
                symbol, "sell", amount, params={"positionSide": Sside}
            )
            logging.info(order)
        except ccxt.InsufficientFunds as e:
            logging.debug(e)
        ...
        if USESL:
            if currentMODE["dualSidePosition"]:
                orderSL = exchange.create_order(
                    ...
                    float(swinghigh),
                    params={
                        "stopPrice": float(swinghigh),
                        "triggerPrice": float(swinghigh),
                        "positionSide": Sside,
                    },
                )
                if Tailing_SL:
                    ordertailingSL = exchange.create_order(
                        ...
                        amount,
                        params={
                            "activationPrice": float(RR1(df, symbol, False)),
                            "callbackRate": float(callbackRate(df)),
                            "positionSide": Sside,
                        },
                    )
        ...
        if USETP:
            orderTP = exchange.create_order(
                ...
                "buy",
                amttp1,
                float(RRTP(df, symbol, False, 1)),
                params={
                    "stopPrice": float(RRTP(df, symbol, False, 1)),
                    "triggerPrice": float(RRTP(df, symbol, False, 1)),
                    "positionSide": Sside,
                },
            )
            logging.info(orderTP)
            orderTP2 = exchange.create_order(
                ...
                "buy",
                amttp2,
                float(RRTP(df, symbol, False, 2)),
                params={
                    "stopPrice": float(RRTP(df, symbol, False, 2)),
                    "triggerPrice": float(RRTP(df, symbol, False, 2)),
                    "positionSide": Sside,
                },
            )
            logging.info(orderTP2)
...
def fetchbars(symbol, timeframe):
    bars = 2002
    print(
        f"Benchmarking new bars for {symbol , timeframe , dt.now().isoformat()}"  # noqa:
    )
    try:
...
def main():
    ...
    if str(local_time[14:-9]) == "0" and not aldynoti:
        ...
        notify.send(
            f"Total Balance : {total} USDT", sticker_id=10863, package_id=789
        )
        aldynoti = True
...
def get_dailytasks():
    ...
    for symbol in symbolist:
        ...
        try:
            ask = float(
                exchange.fetchBidsAsks([symbol])[symbol]["info"]["askPrice"]
            )
        except Exception as e:
            logging.info(f"{e}")
            time.sleep(2)
        ...
        print(
            symbol,
            f"Long_Term : {score1} , Mid_Term : {score2} , Short_Term : {score3}",  # noqa:
        )
        candle(df1, symbol, "1d")
        candle(df3, symbol, "1h")
        dfday = dfday.append(
            pd.Series([symbol, ask, score1, score2, score3], index=daycollum),
            ignore_index=True,
        )
        time.sleep(5)
    return dfday
